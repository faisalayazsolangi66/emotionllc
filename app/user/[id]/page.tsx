            ...
            try {
                ...
                if (data.status && data.data) {
                    ...
                    const mappedProfile = {
                        ...
                        lastActive: "Active 2h ago", // dummy
                        isOnline: true, // dummy
                        isVerified: true, // dummy
                        images: u.albums?.[0]?.photos?.map((p: any) => p.photo_url) || [],
                        compatibility: 87, // dummy
                        mutualInterests: 5, // dummy
                        mutualFriends: 3, // dummy
                        bio: "I love traveling, photography, and cooking. Looking for genuine connections.", // dummy
                        interests: ["Travel", "Photography", "Music", "Cooking", "Fitness", "Movies"], // dummy
                        photos: {
                            total: u.albums?.[0]?.photos?.length || 0,
                            recent: u.albums?.[0]?.photos?.map((p: any) => ({
                                url: p.photo_url,
                                caption: p.description || "No caption",
                            })) || [],
                        },
                        details: {
                ...
                <div className="max-w-6xl mx-auto">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                            ...
                            <div className="grid grid-cols-3 gap-2">
                                {userProfile.images.map((image: string, index: number) => (
                                    <button
                                        ...
                                        className={`relative rounded-lg overflow-hidden border-2 transition-colors ${currentImageIndex === index ? "border-red-500" : "border-transparent"
                                            }`}
                                    >
                                        <img
                        ...
                        <div className="lg:col-span-2 space-y-6">
                            ...
                            {/* Tabs Content (same as your version, but now uses userProfile state) */}
                            {/* ... keep your existing Tabs with mapped userProfile values ... */}
